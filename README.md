...
* `addon/__init__.py`: addon entrypoint, log clear on start, update detection (auto-open Support tab), hook registration.
* `addon/state.py`: shared config state, defaults, constants, `SUPPORT_ITEMS`.
* `addon/logger.py`: debug logger (`debug.log`, 2 MiB rotation, 3 backups), `clear_log_on_startup()`.
* `addon/mapping.py`: mapping validation, preset helpers, and cloze stripping.
* `addon/conversion_dialog.py`: main conversion dialog UI.
* `addon/config_dialog.py`: addon config window orchestrator (inherits `SupportTabMixin`).
* `addon/config_tabs/`: each tab in its own file — `general.py`, `presets.py`, `mappings.py`, `log.py`, `tab_support.py` (+ `widgets.py` for `ADDON_PACKAGE`).
* `addon/operations.py`: note conversion logic and custom undo handling.
* `addon/browser_actions.py`: Browser menu actions and quick-convert integration.
* `addon/reviewer_actions.py`: Reviewer context-menu actions.

See [docs/architecture.md](docs/architecture.md) and [docs/configuration.md](docs/configuration.md) for full architecture, variables, defaults and JSON keys. Development notes have moved to [docs/development.md](docs/development.md).

## Documentation

* [Architecture](docs/architecture.md) — module graph, data flows, hooks.
* [Configuration](docs/configuration.md) — all `config.json` / `meta.json` keys, defaults, schemas.
* [Development](docs/development.md) — versioning, build, symlink testing.
